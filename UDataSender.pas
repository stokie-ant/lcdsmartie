...
uses
  Windows, Messages, SysUtils, DataThread, IdGlobal, IdContext,
  IdBaseComponent, IdComponent, IdTCPConnection, IdTCPClient, IdSSLOpenSSL,IdStackConsts;
   ...
   begin
    ...
    begin
      for Loop := 1 to 4 do begin
        ...
        try
          recv := IdTCPClient1.IOHandler.ReadLn();
        except
          on E : Exception do
          begin
             RLine[1] := E.Message;
         ...
         if not (strtoint(recv[1]) = Loop) then
           Continue;
           Delete(recv,1,1);
           RLine[Loop] := recv;
         end;
