 ...
 *
 *  $Source: /root/lcdsmartie-cvsbackup/lcdsmartie/_SSL/Attic/UConfig.pas,v $
...
type
  ...
  TPopAccount = Record
    ...
    user: String;
    pword: String;
    port_ssl: String; //added for ssl 13/03/2006 by vcorp
  end;
...
var
  ...
  configline: String;
  configArray: Array[1..101] of String; //13/03/2006 from 100 to 101 fos ssl control by vcorp
begin
...
// Pop accounts
  ...
  pop[1].port_ssl := copy(configArray[101], 1, pos('¿0', configArray[101])-1);    //added by vcorp SSL 
  ...
  // Readonly settings - not set at all.
{
  ...
  begin
    ...
    pop[MailCount].server := initFile.ReadString('POP Accounts', 'Server' + sPOPAccount, '');
    pop[MailCount].user := initFile.ReadString('POP Accounts', 'User' + sPOPAccount, '');
    pop[MailCount].pword := initFile.ReadString('POP Accounts', 'Password' +  sPOPAccount, '');
    pop[MailCount].port_ssl := initFile.ReadString('POP Accounts', 'Port_ssl' + sPOPAccount, '');
  end;
...
begin
  ...
  begin
    sPOPAccount := Format('%.2u', [MailCount], localeFormat);
    initFile.WriteString('POP Accounts', 'Server' + sPOPAccount, pop[MailCount].server);
